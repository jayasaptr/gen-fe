...
const BarangKeluarPage = () => {
  ...
  const handleDelete = () => {
    if (eventData) {
      handleDeleteDataBarangKeluar(eventData.id);
  ...
  const validation: any = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      id_pemasok: Yup.string().required("Pelanggan harus diisi!"),
    ...
    onSubmit: (values) => {
      ...
      if (isEdit) {
        handleUpdateBarangKeluar(values);
      } else {
  ...
  const fetchDataBarangKeluar = async () => {
    setLoadingV(true);
    try {
      const userResponse = await axiosInstance.get("/barang-keluar", {
        headers: {
  ...
  const handlePostBarangKeluar = async (data: any) => {
    ...
    try {
      ...
      formData.append("id_pelanggan", data.id_pemasok);
      formData.append("jumlah_keluar", data.jumlah_masuk);
      formData.append("harga_jual", data.harga_satuan);
  ...
  const handleUpdateBarangKeluar = async (data: any) => {
    try {
      setIsLoading(true);
      const formData = new FormData();
      formData.append("id_barang", data.id_barang);
      ...
      Error("Data Barang Keluar Gagal Diupdate");
      if (error.response.status === 401) {
        localStorage.removeItem("authUser");
        naviagate("/login");
      }
  ...
  const handleDeleteDataBarangKeluar = async (id: any) => {
    try {
      setIsLoading(true);
      const userResponse = await axiosInstance.delete(`/barang-keluar/${id}`, {
        headers: {
          Authorization: `Bearer ${user.token}`,
        },
      });
      ...
      >
        ...
        >
          <Modal.Title className="text-16">
            {!!isEdit ? "Edit Barang" : "Add Barang Keluar"}
          </Modal.Title>
        </Modal.Header>
