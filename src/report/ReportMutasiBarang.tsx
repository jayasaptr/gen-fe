...
const ReportMutasiBarang = () => {
    ...
    const fetchDataBarangMasuk = async () => {
        setLoadingV(true);
        try {
            const userResponse = await axiosInstance.get("/mutasi-barang", {
                headers: {
    ...
    }, [search, idKondisi, startDate, endDate]);

    const kategoriUnik = Array.from(
        new Set(data.map((item) => item['id_barang']['kategori']))
    );
    ...
    return (
        <>
            ...
            >
              ...
              <option value="disetuji">Approve</option>
                        ...
                        >
                            ...
                            {kategoriUnik.map((item: any, index: number) => (
            ...
            <div style={{ display: "none" }}>
                <ReportPrint ref={printRef} title="Report Pelanggan">
                    <TableContainer
                        isPagination={false}
                        isTfoot={false}
                        isSelect={false}
...
export default ReportMutasiBarang;
